...
public class ProductPurchase {
        ...
        try {
            ...
            try (Statement stmt = postgresConnection.createStatement()) {
                ...
                while (rs.next()) {
                    ...
                    List<Purchase> purchaseList = orderMap.get(orderId);
                    if (purchaseList == null) {
                        purchaseList = new ArrayList<>();
                        orderMap.put(orderId, purchaseList);
                    }
    ...
    public void exportPurchaseHistoryToExcel(String filePath) throws IOException {
        lock.lock();
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            for (Map.Entry<Integer, List<Purchase>> entry : orderMap.entrySet()) {
                ...
                Row orderRow = sheet.createRow(rowId++);
                Cell orderCell = orderRow.createCell(0);
                orderCell.setCellValue("Order ID: " + orderId);

                for (Purchase purchase : purchases) {
                    Row row = sheet.createRow(rowId++);
                    row.createCell(0).setCellValue("Product ID: " + purchase.getProductId());
                    row.createCell(1).setCellValue("Name: " + getProductName(purchase.getProductId()));
                    row.createCell(2).setCellValue("Quantity: " + purchase.getQuantity());
                    row.createCell(3).setCellValue("Price: " + purchase.getTotalPrice());
                    row.createCell(4).setCellValue("Category: " + purchase.getCategoryName());
                }
            }

            try (FileOutputStream fileOut = new FileOutputStream(filePath)) {
                workbook.write(fileOut);
                System.out.println("Purchase history exported to Excel file: " + filePath);
            } catch (IOException e) {
                e.printStackTrace();
            }
        } finally {
